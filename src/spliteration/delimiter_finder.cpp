#include "delimiter_finder.hpp"



std::string delimiter_finder::get_finded_delimiter_from_current_file(const std::string& path_to_the_file)
{
	std::string result = "";
	...
	if (file_to_analyze.is_open())
	{
		...
		// array of count delimiters length
		// where index++ == real delimiter length
		std::array<size_t, _max_multiple_delimiter_length> count_delimiters_per_length{0};
